...
LINEのヒアリングが完了すると、次の順で自動実行します。

1. 「いただいた内容をもとに広報案を作成します」と返信します。
2. その会話の広報材料から原稿を作成し、媒体別の調整・ビジュアル作成・リスク確認を行います。
3. 完成後、依頼したLINEユーザーへ完了メッセージと承認カードをまとめてプッシュ送信します。
4. カードから承認・修正・保留・却下を選べます。「詳細を確認」は原稿のページを直接開きます。

取得ボタンの操作や「提案」の送信は不要です。「提案」は過去の承認待ち原稿を再表示するために使えます。
このLINE経由の制作は、媒体の自動投稿設定にかかわらず人の承認を待ちます。
「ありがとうございます」「了解です」などの短い返答は、広報材料や新規の制作依頼として扱いません。作成中であれば完成通知を待つ案内を返します。修正カードを選んだ後の文面は、その広報案の修正指示として保存し、新しい広報案は作成しません。
非公開の材料、停止中の広報対象、危機広報モードでは作成を見送り、その理由をLINEへ通知します。
リスク確認で停止した原稿は、承認カードの代わりに確認が必要な旨を通知します。

Webhookは先にHTTP 200を返し、Next.jsの [`after()`](https://nextjs.org/docs/app/api-reference/functions/after) 内で返信と制作を継続します。
実行上限は300秒です。Vercelでは [Fluid computeと実行時間設定](https://vercel.com/docs/functions/configuring-functions/duration) を確認してください。
これは永続ジョブキューではないため、プロセス停止や実行上限超過からの自動再開は行いません。
LINEプッシュの一時的な通信エラー・5xxは同じリトライキーで最大3回試行し、最終的な失敗はサーバーログと通知の `sent_to_line=false` に残します。
LINE側の送信受付は端末への配達・既読を保証するものではありません。

必要な設定は既存のLINEキー、Supabase、AI接続と、公開HTTPS URLの `NEXT_PUBLIC_APP_URL` です。追加のDBマイグレーションはありません。
変更を公開するにはアプリの再デプロイが必要です。

外部APIへ送信せずに自動フローを検証するには `npm run test:line` を実行します。
